		...
		if (_skillNum == 4)
		{
			...
			for (int i = 1; i < _vSkill.size(); i++)
			{
				_vSkill.at(i).skillImage->frameRender(getMemDC(),
					_vSkill.at(0).skillImage->getX() + RND->getFromIntTo(0,400)+ m_camera->getCameraX(),
					_vSkill.at(0).skillImage->getY() - _vSkill.at(0).skillImage->getFrameHeight() / 2 + RND->getFromIntTo(-200, 300) + m_camera->getCameraY());
			}
		}
		// 메테오
		if (_skillNum == 5)
...
void playerSkill::skillSetting(int skillNum,int motionNum)
{
	...
	{ 
		...
		for (int i = 0; i < 5; i++)
		{
			...
			_tagSkill.skillRc = RectMake(_tagSkill.skillImage->getX() + m_camera->getCameraX(), _tagSkill.skillImage->getY() + m_camera->getCameraY(),
				_tagSkill.skillImage->getFrameWidth(), _tagSkill.skillImage->getFrameHeight());
