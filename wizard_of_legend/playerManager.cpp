...
void playerManager::keyManager()
{
	...
	// 왼쪽 스킬 
	if (!m_player->getPlayerLButtonSkill().isActive && m_player->getPlayerLButtonSkill().itemKind != item_Empty && !_isShowPlayerInventory && !_isShowPlayerStatus &&  KEYMANAGER->isOnceKeyDown(VK_LBUTTON) && !_isDash && !_isDashPause && !_playerSkill->getIsSkillActive() && !_playerSkill->getIsPlayerSkillActive())
	{
...
void playerManager::checkEquipItem()
{
	...
	//m_player->setHp(m_player->getHp() + m_player->getPlayerEqiupWeapon().hp + m_player->getPlayerEqiupArmor().hp );
